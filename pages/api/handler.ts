...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    if (req.method === 'GET') {
        ...
        if (!query || !startDate || !endDate) {
            return res.status(400).send('Missing query, startDate, or endDate parameter');
        }

        try {
            // Abrufen von Artikeln von beiden Quellen
            const [presseportalArticles, berlinArticles] = await Promise.all([
                fetchArticlesFromPresseportal(query as string, startDate as string, endDate as string),
                fetchArticlesFromBerlin(query as string, startDate as string, endDate as string),
            ]);
            ...
            const uniqueArticles = combinedArticles.filter((article, index, self) =>
                index === self.findIndex((a) => a.fullArticleURL === article.fullArticleURL)
            );
