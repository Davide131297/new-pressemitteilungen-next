...
export default function ArticleCart({ selectedNews }: ArticleCartProps) {
  const alertArticle = async (item: NewsItem, event: React.MouseEvent) => {
    ...
    <>
      {selectedNews.map((item) => (
        <Link
          ...
          legacyBehavior
        >
          <a onClick={(event) => handleArticleClick(item, event)}>
            <article className="cursor-pointer bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow relative flex flex-col justify-between h-full">
              <div>
                <div className="relative">
                  ...
                  <div
                    ...
                    onClick={(event) => alertArticle(item, event)}
                    title="Melden"
                  >
