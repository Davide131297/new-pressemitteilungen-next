...
import { convertDateFormat } from '@/helper/date-converter';

const fetchArticlesFromBerlin = async (searchtext, startdate, enddate) => {
  const baseURL =
    'https://www.berlin.de/presse/pressemitteilungen/index/search';
  const formattedStartDate = convertDateFormat(startdate);
  const formattedEndDate = convertDateFormat(enddate);
  const searchURL = `${baseURL}?searchtext=${encodeURIComponent(
    searchtext
  )}&boolean=0&startdate=${formattedStartDate}&enddate=${formattedEndDate}&bt=#searchresults`;

  const results = [];
  try {
    const response = await axios.get(searchURL, { timeout: 20000 }); // Timeout auf 20 Sekunden erhöhen
